...
class Taskset(TasksetBase, table=True):
    ...
    projects: List["Project"] = Relationship(back_populates="tasksets", link_model=ProjectTasksetLink)
...
class Project(ProjectBase, table=True):
    id: Optional[int] = Field(default=None, primary_key=True)
    tasksets: List["Taskset"] = Relationship(back_populates="projects", link_model=ProjectTasksetLink)
...
class ProjectReadWithTasksets(ProjectRead):
    tasksets: List[TasksetRead]

class TasksetReadWithProjects(TasksetRead):
    projects: List[ProjectRead]

# TODO make a "WithAll" model including users and tasksets
